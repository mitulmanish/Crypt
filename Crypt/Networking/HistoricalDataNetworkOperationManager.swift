...
struct HistoricalDataNetworkOperationManager {
    ...
    func requestCoinHistoricalData(
        ...
        opeartionQueue.addOperations(
            [oldPriceOperation,
             oldPriceDecodingOperation,
             latestPriceOperation,
             latestPriceDecodingOperation
            ], waitUntilFinished: false
        )

        latestPriceDecodingOperation.completionBlock = {
            if let oldPrice = oldPriceDecodingOperation.decodedObject?.finalPrice,
                let latestPrice = latestPriceDecodingOperation.decodedObject?.finalPrice {
